'use strict';
const parsePopulation = (text)=>/^\d+$/.test(text.replace(/,/g, '').trim()) ? parseInt(text.replace(/,/g, '').trim(), 10) : 0;
...
const populationNumbers = Array.from(document.querySelectorAll('.population')).map((el)=>el.textContent).map(parsePopulation);
